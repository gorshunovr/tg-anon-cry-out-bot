...
func processUpdates(ctx context.Context, bot *tgbotapi.BotAPI, openaiClient *openai.Client, channelName string, updates tgbotapi.UpdatesChannel) {
	rulesMessage := "📝 Правила отправки сообщений:\n\n" +
		"1. Сообщение должно быть преимущественно на русском языке.\n" +
		"2. Запрещены грубые матерные выражения.\n" +
		"3. Допускаются умеренные ругательства с заменой букв на '*'.\n" +
		"4. Цель сообщения – выплеснуть эмоции и получить поддержку."
